...
namespace CGL { namespace SceneObjects {
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    // Vector2D thetaPhi = dir_to_theta_phi(*wi);
    // Vector2D xy = theta_phi_to_xy(thetaPhi);

    // return bilerp(xy);

    // importance sampling
    Vector2D uniform_sample = sampler_uniform2d.get_sample();
    int y_sample = std::upper_bound(marginal_y, marginal_y+envMap->h, uniform_sample.y) - marginal_y;
    int x_sample = std::upper_bound(conds_y + y_sample * envMap->w, conds_y + (y_sample+1) * envMap->w, uniform_sample.x) - (conds_y + y_sample * envMap->w);

    Vector2D sample = Vector2D(x_sample, y_sample);
    Vector2D sample_theta_phi = xy_to_theta_phi(sample);
    *wi = theta_phi_to_dir(sample_theta_phi);
    *distToLight = INF_D;
    *pdf = (float)pdf_envmap[y_sample*envMap->w + x_sample] * envMap->w * envMap->h / (2 * PI * PI * sin(sample_theta_phi.x));
    return envMap->data[envMap->w*y_sample + x_sample];
  ...
  Vector3D EnvironmentLight::sample_dir(const Ray& r) const {
    ...
    // return Vector3D();
    Vector2D theta_pi = dir_to_theta_phi(r.d);
    Vector2D xy = theta_phi_to_xy(theta_pi);
    return bilerp(xy);
  }
